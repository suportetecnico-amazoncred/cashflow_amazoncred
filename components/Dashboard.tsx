...
const Dashboard: React.FC<DashboardProps> = ({ transactions, clients }) => {
  ...
  const installmentsData = useMemo(() => {
    ...
    return {
      ...
      percentage: loanStats.total > 0 ? (((loanStats.total - loanStats.remaining) / loanStats.total) * 100) : 0
    };
  }, [loanStats]);
  ...
  const StatsCard = ({ title, value, icon, color }: { title: string, value: string, icon: string, color: string }) => (
    <div className="bg-white p-5 md:p-6 rounded-2xl md:rounded-3xl shadow-sm border border-slate-100 flex items-center md:block">
  ...
  return (
    <div className="space-y-6 md:space-y-10 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <StatsCard title="Dívida do Plano" value={formatCurrency(loanStats.remaining)} icon="📉" color="text-red-600 bg-red-600" />
        <StatsCard title="Poupança" value={formatCurrency(activeClient.savings)} icon="🐷" color="text-indigo-600 bg-indigo-600" />
        <StatsCard title="Parcelas Restantes" value={`${installmentsData.remaining} / ${loanStats.count}`} icon="📅" color="text-amber-600 bg-amber-600" />
      </div>
          ...
          <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-3xl shadow-sm border border-emerald-100 relative overflow-hidden">
             ...
             <div className="space-y-4 relative z-10">
                ...
                <div className="h-4 bg-slate-100 rounded-full overflow-hidden flex gap-1 p-1">
                   {loanStats.count > 0 ? Array.from({ length: loanStats.count }).map((_, i) => (
                     <div 
                      key={i} 
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-3xl border border-slate-200 shadow-sm">
             ...
             <div className="w-full bg-emerald-50 text-emerald-600 p-3 rounded-xl text-center font-bold text-[10px] border border-emerald-100 uppercase">
                {loanStats.remaining > 0 ? 'Situação: Em aberto' : 'Situação: Quitado ✅'}
             </div>
          </div>
        </div>
      </div>
