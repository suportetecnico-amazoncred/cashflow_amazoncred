  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-6 duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        {hasActiveLoan ? (
          <div className="bg-white p-10 rounded-[2.5rem] shadow-xl border-2 border-amber-100 flex flex-col items-center justify-center text-center space-y-6 lg:col-span-2">
            ...
            <div className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100">
               ...
               <div className="flex justify-between mt-2 mb-1 text-[10px] font-bold text-emerald-600">
                 <span>{activeClient.loanTotal > 0 ? (((activeClient.loanTotal - activeClient.creditUsed) / activeClient.loanTotal) * 100).toFixed(0) : 0}% Pago</span>
               </div>
               <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                 <div 
                   className="h-full bg-emerald-500" 
                   style={{ width: `${activeClient.loanTotal > 0 ? ((activeClient.loanTotal - activeClient.creditUsed) / activeClient.loanTotal) * 100 : 0}%` }}
                 ></div>
               </div>
            </div>
          </div>
